...
async function startJob(type, payload={}){
  ...
  const int=setInterval(async()=>{
    ...
    const lg=await fetch('/logs/'+job_id); log.textContent=await lg.text(); log.scrollTop=log.scrollHeight;
    if(['finished','failed','killed'].includes(js.status)){ clearInterval(int); jobs(); } },2000);
  jobs();
}

function pad(x){return String(x).padStart(2,'0');} function val(id){return document.getElementById(id).value;}

function openDateDialog(cb){
  ...
  save.onclick=(e)=>{
    ...
    dlg.close(); cb({date_from:`${d.y}-${pad(d.m)}-${pad(d.d)}`,date_to:`${h.y}-${pad(h.m)}-${pad(h.d)}`,account:sel.value});
  };
  cancel.onclick=()=>{try{dlg.close();}catch(e){}};
}

document.getElementById('runTransferBtn').onclick=()=>startJob('transfer');
document.getElementById('runCsvBtn').onclick = ()=>openDateDialog(df=>startJob('csv',df));
